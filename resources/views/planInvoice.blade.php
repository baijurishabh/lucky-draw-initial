...
<body>
    ...
    <table class="ft" style="width:100%; border: 1px solid black;border-collapse: collapse; margin-top:20px;">
        <tr>
            <td style="border: 1px solid black; border-collapse: collapse; padding:10px;">
                ...
                <span style="font-size: 13px"> <b> {{ $payment->user->name }}</b></span><br>
                <span style="font-size: 13px"> {{ $payment->user->userDetails->address_line1 }}, {{ $payment->user->userDetails->address_line2 }}, {{ $payment->user->userDetails->state }},
                    {{ $payment->user->userDetails->country }}</span><br>
                <span>
                    <img src="{{ public_path('/assets/invoice/icons/call icon.png') }}" alt="ph" srcset=""
                        style="width:10px; height:10px;">
                    <span style="font-size: 13px"> {{ $payment->user->mobile }}</span>
                    <img src="{{ public_path('/assets/invoice/icons/mail.png') }}" alt="email" srcset=""
                        style="width:10px; height:10px;">
                    <span style="font-size: 13px"> {{ $payment->user->email }}</span>
                </span>
            </td>
            <td style="border: 1px solid black;border-collapse: collapse;padding:10px;">
                <span style="font-size: 13px">Payment ID.</span> <br>
                <span style="font-size: 14px">{{ $payment->rzp_payment_id }}</span><br>

                <span style="font-size: 13px; padding-top:10px;">Date.</span> <br>
                <span style="font-size: 14px">
                    {{ \Carbon\Carbon::parse($payment->created_at)->format('j F, Y') }}</span>
            ...
            <td style="border: 1px solid black;border-collapse: collapse;padding:10px;">
                ...
                <span style="font-size: 20px"><b>₹{{ $payment->amount }}/-</b></span>
    ...
    <table class="st" style="width:100%; margin-top:30px; ">
        ...
        <tr>
            <td class="st-d" style="padding: 10px "><span>Account Registration Fee</span></td>
            <td class="st-d" style="padding: 10px"><span>₹{{ $payment->amount }}</span></td>
            <td class="st-d" style="padding: 10px"><span>₹{{ $payment->amount }}</span></td>
            </td>
        </tr>
        ...
        border-collapse: collapse; padding-bottom:10px">
            ...
            <tr>
                <td style="border-bottom: none; padding:10px ">
                    <span style="font-size:15px "> Payment Type : {{ $payment->payment_type }}</span><br>
                    <span style="font-size: 12px ">Payment Method : {{ $payment->mode }}</span>
                </td>
            </tr>
            <tr>
                <td style="border-bottom: none; padding:10px">

                    <span style="font-size: 15px">Payment ID : {{ $payment->rzp_payment_id }}</span><br>
<br>
                    <span style="font-size: 15px">Transaction ID : {{ $payment->transaction_id }}</span><br>
        ...
        <table class="tt-s"
            ...
            <tr>
                ...
                <td class="tt-h">₹{{ $payment->amount }}</td>
